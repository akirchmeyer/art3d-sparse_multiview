...
from ddim_inv_scheduler import DDIMInverseScheduler, DDIMSchedulerOutput
...
class DDIMInversion(BasePipeline):
    ...
    def __call__(
        ...
        for i, t in enumerate(timesteps.flip(0)[1:-1]):
            latents = self.step(latents, t, prompt_embeds, cross_attention_kwargs, guidance_scale, extra_step_kwargs, lambda_ac, lambda_kl, num_reg_steps, num_ac_rolls)
    ...
    def step(
        self, latents, t, prompt_embeds, cross_attention_kwargs, guidance_scale=7.5, extra_step_kwargs,
        # inversion regularization parameters
        lambda_ac: float = 20.0,
        lambda_kl: float = 20.0,
        num_reg_steps: int = 5,
